...
def create_approved_file(client, mis_headers, admin_headers):
    ...
    approved = client.post(f"/api/files/{file_id}/approve", headers=admin_headers)
...
class TestStats:
    def test_stats_requires_auth(self, client):
        resp = client.get("/api/query/stats")
    ...
    def test_stats_returns_counts(self, client, employee_headers, mis_headers, admin_headers):
        ...
        resp = client.get("/api/query/stats", headers=employee_headers)
    ...
    def test_stats_limit_clamped(self, client, employee_headers):
        resp = client.get("/api/query/stats?limit=9999", headers=employee_headers)
        assert resp.status_code == 200
...
class TestRawData:
    def test_raw_data_requires_auth(self, client):
        resp = client.post("/api/query/raw-data", json={"table_name": "foo"})
        assert resp.status_code == 401
...
class TestInsights:
    def test_insights_empty_system(self, client, employee_headers):
        resp = client.get("/api/query/insights", headers=employee_headers)
        assert resp.status_code == 200
        body = resp.json()
